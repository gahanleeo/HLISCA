...
peak_info <- read.csv("peaks_by_cell_types_CHL_chranno.csv", header = TRUE, row.names = 1)
...
setdiff(old_ccv, GWAS_subset$RSNUM)

tmp <- lung_GWAS[which(lung_GWAS$RSNUM %in% setdiff(old_ccv, GWAS_subset$RSNUM)),]
